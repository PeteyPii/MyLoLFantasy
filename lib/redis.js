...
function Redis() {
  ...
  self.resetPasswordAllowed = function(ip, username) {
    var promises = [
      Q.ninvoke(self.client, 'evalsha', self.throttleSha, 1, PASSWORD_RESET_COUNT_KEY, IP_PREFIX + ip, settings.password_reset_burst_period),
      Q.ninvoke(self.client, 'evalsha', self.throttleSha, 1, PASSWORD_RESET_COUNT_KEY, USER_PREFIX + username, settings.password_reset_burst_period),
    ];
    return Q.all(promises).spread(function(ipCount, usernameCount) {
      return ipCount <= settings.password_reset_burst_count && usernameCount <= settings.password_reset_burst_count;
    });
  };
