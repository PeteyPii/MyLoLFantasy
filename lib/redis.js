...
function Redis() {
  ...
  self.logInAllowed = function(ip, username) {
    var promises = [
      Q.ninvoke(self.client, 'evalsha', self.throttleSha, 1, LOG_IN_COUNT_KEY, ip, settings.failed_log_in_reset_time),
      Q.ninvoke(self.client, 'evalsha', self.throttleSha, 1, LOG_IN_COUNT_KEY, username, settings.failed_log_in_reset_time),
    ];
  ...
  self.logInSuccess = function(ip, username) {
    var promises = [
      Q.ninvoke(self.client, 'evalsha', self.throttleUndoSha, 1, LOG_IN_COUNT_KEY, ip),
      Q.ninvoke(self.client, 'evalsha', self.throttleUndoSha, 1, LOG_IN_COUNT_KEY, username),
    ];
  ...
  self.leagueCreationAllowed = function(ip, username) {
    var promises = [
      Q.ninvoke(self.client, 'evalsha', self.throttleSha, 1, LEAGUE_CREATION_COUNT_KEY, ip, settings.league_creation_burst_period),
      Q.ninvoke(self.client, 'evalsha', self.throttleSha, 1, LEAGUE_CREATION_COUNT_KEY, username, settings.league_creation_burst_period),
    ];
    return Q.all(promises).spread(function(ipCount, usernameCount) {
