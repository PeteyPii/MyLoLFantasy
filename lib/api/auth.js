...
router.post('/log-in', apiHelpers.verifyCsrfToken, function(req, res) {
...
router.post('/log-out', apiHelpers.verifyCsrfToken, function(req, res) {
  req.logout();
  res.send({
    success: true,
...
router.post('/sign-up', apiHelpers.verifyCsrfToken, function(req, res) {
  var username = req.body.username;
  var password = req.body.password;
  ...
  req.app.locals.redis.signUpAllowed(req.ip).then(function(allowed) {
    ...
    if (user) {
      ...
      return req.app.locals.lol.getBothIds(region, summonerName).fail(function(err) {
        if (err.status) {
  ...
  }).then(function(bothIds) {
    if (bothIds.summonerId === -1 || bothIds.accountId === -1) {
...
router.post('/reset-password', apiHelpers.verifyCsrfToken, function(req, res) {
  var username = req.body.username;
  ...
  req.app.locals.redis.resetPasswordAllowed(req.ip, username).then(function(allowed) {
    ...
    if (user) {
      ...
      return req.app.locals.redis.createResetPasswordToken(username, token).then(function() {
        if (user.email) {
          var link = 'https://' + req.hostname + settings.base_path + 'reset-password/' + token;
...
router.post('/reset-password/:token', apiHelpers.verifyCsrfToken, function(req, res) {
  var token = req.params.token;
  var password = req.body.password;
